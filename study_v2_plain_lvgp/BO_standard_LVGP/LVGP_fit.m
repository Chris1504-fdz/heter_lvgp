...
addOptional(InParse, 'ub_phi_lat', 3);
addOptional(InParse, 'lb_z', -3);
addOptional(InParse, 'ub_z', 3);
addOptional(InParse, 'n_opt', 8);
addOptional(InParse, 'parallel', 0);
addOptional(InParse, 'max_iter_ini', 500);
addOptional(InParse, 'max_iter_lat', 100);
addOptional(InParse, 'max_eval_ini', 5000);
addOptional(InParse, 'max_eval_lat', 1000);
addOptional(InParse, 'seed', 123);
...
if isempty(ind_qual)
    ...
    for i = 1:p_qual
        ...
        n_lvs_qual(i) = length(lvs_qual{i}); % number of levels for the i-th qualitative factor
    end
...
options_ini = optimoptions(@fmincon, 'Algorithm', 'interior-point', ...
    'display', 'notify', 'MaxIterations', max_iter_ini,...
    'MaxFunctionEvaluations', max_eval_ini);
    %'ScaleProblem','obj-and-constr',...
    %'FiniteDifferenceType', 'central');
options_lat = optimoptions(@fmincon, 'Algorithm', 'interior-point', ...
    'display', 'notify', 'MaxIterations', max_iter_lat,...
    'MaxFunctionEvaluations', max_eval_lat);
    %'ScaleProblem','obj-and-constr',...
    %'FiniteDifferenceType', 'central');
...
setting = struct('max_iter_ini', max_iter_ini, 'max_iter_lat',max_iter_lat,...
    ...
    'parallel',parallel, 'eps_all',eps_all);
